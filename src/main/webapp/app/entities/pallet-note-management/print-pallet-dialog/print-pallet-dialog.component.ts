...
export class PrintPalletDialogComponent implements OnInit {
  ...
  async onExportPdf(): Promise<void> {
    ...
    if (unprintedPallets.length === 0) {
      this.snackBar.open(
        "Không có phiếu nào cần xuất PDF!",
        "Đóng",
        { duration: 3000 }
      );
    ...
    try {
      ...
      for (let batchStart = 0; batchStart < pageElements.length; batchStart += BATCH_SIZE) {
        ...
        const canvasPromises = batch.map(pageEl => {
          // Tắt effects
          const cards = pageEl.querySelectorAll(".pallet-card") as NodeListOf<HTMLElement>;
          cards.forEach(card => {
        ...
        for (let i = 0; i < canvases.length; i++) {
          ...
          if (pageIndex > 0) {
            pdf.addPage(isA5 ? [148, 210] : [297, 210], isA5 ? "portrait" : "landscape");
          }
      ...
      this.snackBar.open(
        `Xuất ${unprintedPallets.length} phiếu thành công!`,
        "Đóng",
        { duration: 2000 }
      );
    } catch (err) {
