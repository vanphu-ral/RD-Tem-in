...
export class LenhSanXuatUpdateComponent implements OnInit {
  ...
  guiPheDuyet(): void {
    ...
    this.lenhSanXuatService.update(lenhSanXuat).subscribe(() => {
      ...
      this.http
        .put<any>(
          `${this.resourceUrl1}/${this.editForm.get(["id"])!.value as number}`,
  ...
  changeQuantity(): void {
    ...
    for (let i = 0; i < this.chiTietLenhSanXuats.length; i++) {
      if ((this.chiTietLenhSanXuats[i] as any).trang_thai === "Active") {
        const result = (this.chiTietLenhSanXuats[i] as any).total_quantity;
        if (result) {
  ...
  changeAllStorageUnit(): void {
    for (let i = 0; i < this.chiTietLenhSanXuats.length; i++) {
      (this.chiTietLenhSanXuats[i] as any).storage_code = this.storageUnit;
    }
  }
