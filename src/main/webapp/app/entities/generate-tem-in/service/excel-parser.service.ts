        ...
        try {
          ...
          for (let i = 1; i < jsonData.length; i++) {
            ...
            const excelRow: ExcelImportData = {
              ...
              userData1: this.getCellValue(row[7]),
              userData2: this.getCellValue(row[8]),
              userData3: this.getCellValue(row[9]),
              userData4: this.getCellValue(row[10]),
              userData5: this.getCellValue(row[11]),
              storageUnit: this.getCellValue(row[12]),
              expirationDate: this.parseDate(row[13]),
              manufacturingDate: this.parseDate(row[14]),
              arrivalDate: this.parseDate(row[15]),
            };
